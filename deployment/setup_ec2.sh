...
# 배포 스크립트 권한 설정
if [ -f "/opt/lawfirmai/deployment/deploy.sh" ]; then
    chmod +x /opt/lawfirmai/deployment/deploy.sh
fi

if [ -f "/opt/lawfirmai/deployment/backup.sh" ]; then
...
(crontab -l 2>/dev/null; echo "0 2 * * * /opt/lawfirmai/deployment/backup.sh >> /opt/lawfirmai/logs/backup.log 2>&1") | crontab -
...
echo "3. Copy deployment/docker-compose.prod.yml (or deployment/docker-compose.prod.free-tier.yml for free tier) to /opt/lawfirmai/deployment"
echo "4. Run deployment script: /opt/lawfirmai/deployment/deploy.sh"
echo ""
