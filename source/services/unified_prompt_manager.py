...
"""
    ...
    def _build_final_prompt(self, base_prompt: str, query: str, context: Dict[str, Any], question_type: QuestionType) -> str:
        """최종 프롬프트 구성 - Chain of Thought 우선 방식"""
...
다음 사고 과정을 따라 단계별로 생각하고 답변하세요. 템플릿이나 고정된 형식에 얽매이지 말고, 질문에 맞는 자연스러운 답변을 제공하세요.

### 사고 과정 (내부적으로만 진행)
1. **질문 이해**: 이 질문의 핵심은 무엇인가? 어떤 법률 분야인가?
2. **법적 근거**: 관련 법령과 조문은 무엇인가? 판례는 있는가?
3. **법리 분석**: 법률 해석과 쟁점은 무엇인가? 예외사항은?
4. **실무 적용**: 구체적 절차와 주의사항은 무엇인가?
5. **답변 구성**: 가장 중요한 정보부터 자연스럽게 설명하라
...
- "~하시면 됩니다", "~해 보세요" 같은 자연스러운 조언을 사용하세요
...
## 답변 작성 지침
위 사고 과정을 바탕으로 질문에 직접적으로 답변하세요. 고정된 템플릿이나 형식을 따르지 말고, 질문의 성격에 맞는 자연스러운 답변을 제공하세요.
