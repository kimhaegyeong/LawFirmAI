...
class EnhancedChatService:
    ...
    def __init__(self, config: Config):
        ...
        # 법령용어 검색 서비스 초기화
        self._initialize_legal_term_services()
    ...
    def _initialize_legal_term_services(self):
        """법령용어 검색 서비스 초기화"""
        try:
            # 법령용어 검색 서비스
            from .legal_term_search_service import LegalTermSearchService
            self.legal_term_service = LegalTermSearchService(self.db_manager)
            
            # 동적 키워드 시스템
            from .dynamic_keyword_system import DynamicKeywordSystem
            self.dynamic_keyword_system = DynamicKeywordSystem(self.legal_term_service)
            
            # 캐시 워밍업
            self.legal_term_service.warm_up_cache()
            
            self.logger.info("법령용어 검색 서비스 초기화 완료")
            
        except Exception as e:
            self.logger.error(f"법령용어 검색 서비스 초기화 실패: {e}")
            self.legal_term_service = None
            self.dynamic_keyword_system = None
        ...
        try:
            ...
            if statute_law and statute_article and self.current_law_search_engine:
                try:
                    ...
                    if specific_result and hasattr(specific_result, 'article_content') and specific_result.article_content:
                        return {
                    ...
                    elif specific_result and hasattr(specific_result, 'matched_content') and specific_result.matched_content:
            ...
            if self.unified_rag_service:
                try:
                    ...
                    if rag_response and rag_response.response and self._has_meaningful_sources(rag_response.sources):
                        ...
                        return {
                            "response": self._enhance_response_with_legal_terms(rag_response.response, message),
                            "confidence": confidence,
    ...
    def _has_meaningful_sources(self, sources: List[Dict[str, Any]]) -> bool:
        ...
        # 추가 검증: 실제 법률 관련 내용인지 확인 (동적 키워드 사용)
        if meaningful_sources:
            ...
            for source in meaningful_sources:
                content = source.get("content", "")
                if self._validate_legal_content_dynamic(content):
...
일반적인 법률 상식이나 절차에 대해서는 안내해드릴 수 있습니다."""
    ...
    def _generate_search_suggestions_fallback(self, message: str, query_analysis: Dict[str, Any]) -> List[str]:
        """폴백 검색 제안 생성 (기존 하드코딩된 방식)"""
        suggestions = []
        
        # 질문에서 직접 키워드 추출 (기존 로직)
        import re
    ...
    def _generate_search_suggestions(self, message: str, query_analysis: Dict[str, Any]) -> List[str]:
        ...
        try:
            # 동적 키워드 추출 사용
            keyword_analysis = self._extract_keywords_from_query(message)
            extracted_keywords = keyword_analysis.get("extracted_terms", [])
            
            if not extracted_keywords:
                # 폴백: 기존 방식 사용
                return self._generate_search_suggestions_fallback(message, query_analysis)
            ...
            for keyword in extracted_keywords:
                if keyword in ["민법", "형법", "상법", "노동법", "가족법", "행정법", "헌법"]:
                    priority_keywords.append(keyword)
                elif keyword in ["계약", "손해배상", "이혼", "상속", "범죄", "처벌"]:
                    specific_keywords.append(keyword)
                else:
                    general_keywords.append(keyword)
            ...
            if extracted_keywords:
                main_keyword = extracted_keywords[0]
                suggestions.append(f"'{main_keyword}'에 대한 자세한 정보를 찾아보세요")
                
                if len(extracted_keywords) > 1:
                    suggestions.append(f"'{extracted_keywords[1]}'와 함께 검색해보세요")
                
                if len(extracted_keywords) > 2:
                    suggestions.append(f"'{extracted_keywords[2]}' 관련 조문을 확인해보세요")
            
            # 법령용어 자동완성 제안
            if self.legal_term_service and len(message) >= 2:
                term_suggestions = self.legal_term_service.get_term_suggestions(message[:3], limit=3)
                for suggestion in term_suggestions:
                    if suggestion not in suggestions:
                        suggestions.append(f"'{suggestion}' 관련 법령용어를 검색해보세요")
            
            # 관련 법령용어 제안
            if self.legal_term_service and extracted_keywords:
                related_terms = self.legal_term_service.get_related_terms(extracted_keywords[0], limit=2)
                for term in related_terms:
                    term_name = term['법령용어명']
                    if term_name not in suggestions:
                        suggestions.append(f"'{term_name}' 관련 용어도 확인해보세요")
            
        except Exception as e:
            self.logger.error(f"동적 검색 제안 생성 실패: {e}")
            return self._generate_search_suggestions_fallback(message, query_analysis)
    ...
    def _extract_keywords_from_query(self, query: str) -> Dict[str, Any]:
        """쿼리에서 키워드 추출 (동적 시스템 사용)"""
        try:
            if not self.legal_term_service or not self.dynamic_keyword_system:
                # 폴백: 기존 하드코딩된 방식 사용
                return self._extract_keywords_fallback(query)
            
            # 동적 키워드 시스템 사용
            dynamic_keywords = self.dynamic_keyword_system.update_keywords_dynamically(query)
            
            # 법령용어 추출
            legal_terms = self.legal_term_service.extract_legal_terms_from_text(query)
            
            # 최적 도메인 식별
            best_domain = self.dynamic_keyword_system.get_best_domain(query)
            
            return {
                "dynamic_keywords": dynamic_keywords,
                "legal_terms": legal_terms,
                "extracted_terms": [term["법령용어명"] for term in legal_terms],
                "best_domain": best_domain,
                "domain_score": self.dynamic_keyword_system.calculate_domain_score(best_domain, query)
            }
            
        except Exception as e:
            self.logger.error(f"동적 키워드 추출 실패: {e}")
            return self._extract_keywords_fallback(query)
    
    def _extract_keywords_fallback(self, query: str) -> Dict[str, Any]:
        """폴백 키워드 추출 (기존 하드코딩된 방식)"""
        import re
        ...
        return {
            "dynamic_keywords": {},
            "legal_terms": [],
            "extracted_terms": list(set(extracted_keywords)),
            "best_domain": "general",
            "domain_score": 0.0
        }
    
    def _enhance_response_with_legal_terms(self, response: str, query: str) -> str:
        """응답에 법령용어 정보 추가"""
        try:
            if not self.legal_term_service:
                return response
            
            # 쿼리에서 관련 법령용어 추출
            related_terms = self.legal_term_service.get_related_terms(query, limit=3)
            
            if related_terms:
                term_info = "\n\n📚 관련 법령용어:\n"
                for term in related_terms:
                    term_info += f"• {term['법령용어명']}"
                    if term.get('비고'):
                        term_info += f" - {term['비고']}"
                    term_info += "\n"
            ...
            self.logger.error(f"응답에 법령용어 정보 추가 실패: {e}")
            return response
    
    def _validate_legal_content_dynamic(self, content: str) -> bool:
        """법률 관련 내용 검증 (동적 키워드 사용)"""
        try:
            if not self.legal_term_service:
                # 폴백: 기존 방식 사용
                return self._validate_legal_content_fallback(content)
            
            # 동적 시스템으로 법률 관련 내용 검증
            return self.legal_term_service.validate_legal_content(content)
            
        except Exception as e:
            self.logger.error(f"동적 법률 내용 검증 실패: {e}")
            return self._validate_legal_content_fallback(content)
    
    def _validate_legal_content_fallback(self, content: str) -> bool:
        """폴백 법률 내용 검증 (기존 하드코딩된 방식)"""
...
답변:"""
    ...
    def _classify_domain_by_weight(self, message: str) -> str:
        """가중치 기반 도메인 분류 (동적 시스템 사용)"""
        try:
            if not self.dynamic_keyword_system:
                # 폴백: 기존 방식 사용
                return self._classify_domain_by_weight_fallback(message)
            
            # 동적 시스템으로 최적 도메인 식별
            best_domain = self.dynamic_keyword_system.get_best_domain(message)
            return best_domain
            
        except Exception as e:
            self.logger.error(f"동적 도메인 분류 실패: {e}")
            return self._classify_domain_by_weight_fallback(message)
    
    def _classify_domain_by_weight_fallback(self, message: str) -> str:
        """폴백 도메인 분류 (기존 하드코딩된 방식)"""
    ...
    def _extract_intent_from_message(self, message: str, domain: str) -> str:
        """메시지에서 의도 추출 (동적 시스템 사용)"""
        try:
            if not self.dynamic_keyword_system:
                # 폴백: 기존 방식 사용
                return self._extract_intent_from_message_fallback(message, domain)
            
            # 동적 시스템으로 도메인 키워드 가져오기
            domain_keywords = self.dynamic_keyword_system.get_domain_keywords(domain)
            
            # 의도 추출 로직 (기존과 동일하지만 동적 키워드 사용)
            message_lower = message.lower()
            
            # 질문 의도 키워드
            question_keywords = ["무엇", "어떻게", "왜", "언제", "어디서", "누가", "어떤", "몇", "얼마나"]
            if any(keyword in message_lower for keyword in question_keywords):
                return "question"
            
            # 조언 의도 키워드
            advice_keywords = ["조언", "도움", "방법", "해결", "대처", "어떻게 해야"]
            if any(keyword in message_lower for keyword in advice_keywords):
                return "advice"
            
            # 정보 요청 의도 키워드
            info_keywords = ["알고 싶", "궁금", "설명", "의미", "정의", "개념"]
            ...
            self.logger.error(f"동적 의도 추출 실패: {e}")
            return self._extract_intent_from_message_fallback(message, domain)
    
    def _extract_intent_from_message_fallback(self, message: str, domain: str) -> str:
        """폴백 의도 추출 (기존 하드코딩된 방식)"""
        message_lower = message.lower()
...
답변:"""
    ...
    def _enhance_response_with_legal_terms(self, response: str, query: str) -> str:
        ...
        try:
            # 응답에서 법령용어 추출
            extracted_terms = self.legal_term_service.extract_legal_terms_from_text(response)
            
            if extracted_terms:
                enhanced_response = response + "\n\n**?뱴 愿??踰뺣졊?⑹뼱:**\n"
                for i, term in enumerate(extracted_terms[:3]):  # 상위 3개만 표시
                    term_name = term.get('법령용어명', '')
                    if term_name:
                        # 간단한 정의 제공
                        definition = self._get_simple_definition(term_name)
                        if definition:
                            enhanced_response += f"• **{term_name}**: {definition}\n"
                
                return enhanced_response
            
        except Exception as e:
            self.logger.debug(f"Legal term enhancement failed: {e}")
        
        return response
    
    def _get_simple_definition(self, term_name: str) -> str:
        """踰뺣졊?⑹뼱??媛꾨떒???뺤쓽 ?쒓났"""
        definitions = {
            "遺덈쾿?됱쐞": "怨좎쓽 ?먮뒗 怨쇱떎濡??명븳 ?꾨쾿?됱쐞濡???몄뿉寃??먰빐瑜?媛?섎뒗 ?됱쐞",
            "?먰빐諛곗긽": "遺덈쾿?됱쐞濡??명븳 ?먰빐瑜?諛곗긽?섎뒗 寃?,
            "怨좎쓽": "?됱쐞?먭? 寃곌낵 諛쒖깮???몄떇?섍퀬 ?섏슃?섎뒗 ?щ━?곹깭",
            "怨쇱떎": "二쇱쓽?섎Т瑜??꾨컲???щ━?곹깭",
            "?꾨쾿?됱쐞": "踰뺣졊???꾨컲?섎뒗 ?됱쐞",
            "?멸낵愿怨?: "?먯씤怨?寃곌낵 媛꾩쓽 ?꾩뿰???곌껐愿怨?,
            "怨꾩빟": "?뱀궗??媛꾩쓽 ?섏궗?쒖떆???⑹튂",
            "?먰빐": "踰뺤씡??移⑦빐濡??명븳 ?ъ궛?겶룹젙?좎쟻 遺덉씠??,
            "諛곗긽": "?먰빐瑜??먯긽?뚮났?섎뒗 寃?,
            "梨낆엫": "踰뺣쪧??遺?댄븯???섎Т",
            "?뚯쑀沅?: "臾쇨굔???먯쑀濡?쾶 ?ъ슜, ?섏씡, 泥섎텇?????덈뒗 沅뚮━",
            "?먯쑀沅?: "臾쇨굔???ъ떎??吏諛고븯??沅뚮━",
            "?곸냽": "?щ쭩???먯쓽 ?ъ궛怨?梨꾨Т瑜?踰뺤젙?곸냽?몄씠 ?밴퀎?섎뒗 寃?,
            "?좎뼵": "?щ쭩 ???ъ궛 泥섎텇??????섏궗?쒖떆",
            "?댄샎": "?쇱씤 愿怨꾨? ?댁냼?섎뒗 寃?,
            "?쇱씤": "?⑤?媛 踰뺤쟻?쇰줈 遺遺媛 ?섎뒗 寃?,
            "踰붿즲": "?뺣쾿???섑빐 泥섎쾶諛쏅뒗 ?됱쐞",
            "泥섎쾶": "踰붿즲?????踰뺤쟻 ?쒖옱",
            "?뺣웾": "踰붿즲?????援ъ껜??泥섎쾶 ?섏?",
            "踰뚭툑": "?ъ궛?뺤쓽 ?쇱쥌?쇰줈 ?쇱젙 湲덉븸???⑸??섎뒗 泥섎쾶",
            "吏뺤뿭": "?먯쑀?뺤쓽 ?쇱쥌?쇰줈 援먮룄?뚯뿉 ?섏슜?섎뒗 泥섎쾶",
            "怨쇳깭猷?: "寃쎈????꾨컲?됱쐞??????ъ궛??,
            "?됱젙泥섎텇": "?됱젙湲곌???踰뺣졊??洹쇨굅?섏뿬 ?됲븯??援ъ껜???됱쐞",
            "?덇?": "?됱젙湲곌????뱀젙 ?됱쐞瑜??덉슜?섎뒗 泥섎텇",
            "?뚯궗": "?곷━ 紐⑹쟻?쇰줈 ?ㅻ┰??踰뺤씤",
            "二쇱떇": "二쇱떇?뚯궗???먮낯 援ъ꽦 ?⑥쐞",
            "?댁궗": "二쇱떇?뚯궗???낅Т 吏묓뻾 湲곌?"
        }
        return definitions.get(term_name, "")
