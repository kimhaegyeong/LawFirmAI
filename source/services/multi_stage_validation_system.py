...
class MultiStageValidationSystem:
    ...
    def _make_final_decision(self, total_score: float, stages: List[StageResult], query: str = "") -> ValidationResult:
        ...
        try:
            ...
            import os as _os
            try:
                default_restriction = float(_os.getenv("RESTRICTION_THRESHOLD_DEFAULT", "0.20"))  # 더 관대하게 조정
                default_allowance = float(_os.getenv("ALLOWANCE_THRESHOLD_DEFAULT", "0.10"))     # 더 관대하게 조정
                allowed_restriction = float(_os.getenv("RESTRICTION_THRESHOLD_WITH_ALLOWED", "0.15"))  # 더 엄격
                allowed_allowance = float(_os.getenv("ALLOWANCE_THRESHOLD_WITH_ALLOWED", "0.05"))      # 더 엄격
            ...
            if category in ("illegal_activity_assistance", "criminal_case_advice", "medical_legal_advice"):
                try:
                    restriction_threshold = float(_os.getenv("RESTRICTION_THRESHOLD_SENSITIVE", str(restriction_threshold)))
                    allowance_threshold = float(_os.getenv("ALLOWANCE_THRESHOLD_SENSITIVE", str(allowance_threshold)))
                except Exception:
                    pass
            elif category in ("general_legal_information", "edge_cases"):
                try:
                    restriction_threshold = float(_os.getenv("RESTRICTION_THRESHOLD_GENERAL", str(restriction_threshold)))
                    allowance_threshold = float(_os.getenv("ALLOWANCE_THRESHOLD_GENERAL", str(allowance_threshold)))
                except Exception:
                    pass
