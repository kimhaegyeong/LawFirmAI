...
class LegalVectorStore:
    ...
    def load_index(self, filepath: str = None) -> bool:
        """인덱스와 메타데이터 로딩 - 개선된 버전"""
        try:
            if filepath is None:
                filepath = "data/embeddings/legal_vector_index"

            self.logger.info(f"Loading vector index from: {filepath}")
            filepath = Path(filepath)
            ...
            if filepath.suffix == '':
                ...
                if not Path(faiss_file).exists():
                    ...
                    if not Path(faiss_file).exists():
                        self.logger.error(f"FAISS file not found: {faiss_file}")
                        return False

                self.logger.info(f"Loading FAISS index from: {faiss_file}")
