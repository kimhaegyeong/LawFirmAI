...
class DatabaseManager:
    ...
    새로운 프로젝트는 lawfirm_v2.db 스키마와 LegalDataConnectorV2를 사용하세요.
    """
    ...
    def __init__(self, db_path: str = "data/lawfirm.db"):
        ...
        import warnings
        warnings.warn(
            "DatabaseManager (lawfirm.db) is deprecated. Use lawfirm_v2.db with LegalDataConnectorV2 instead.",
            DeprecationWarning,
            stacklevel=2
        )
        self.db_path = Path(db_path)
        self.db_path.parent.mkdir(parents=True, exist_ok=True)
        logger.warning(f"⚠️ DatabaseManager is deprecated. Migrate to lawfirm_v2.db.")
        self._create_tables()
    ...
    def _create_tables(self):
        ...
        with self.get_connection() as conn:
            ...
            # --- Compatibility views/tables for Text-to-SQL router ---
            # Articles view mapped from assembly tables
            try:
                cursor.execute(
                    """
                    CREATE VIEW IF NOT EXISTS articles AS
                    SELECT al.law_name AS law_name,
                           aa.article_number AS article_number,
                ...
                logger.warning(f"Failed to create view 'articles': {e}")

            # Cases view mapped from precedent_cases
            try:
                cursor.execute(
                    """
                    ...
                    SELECT case_id AS id,
                           ...
                           court,
                           decision_date,
                           substr(full_text, 1, 4000) AS summary
                    FROM precedent_cases
                    """
                )
            except Exception as e:
                logger.warning(f"Failed to create view 'cases': {e}")

            # Laws simple view for router table detection
            try:
                cursor.execute(
                    """
                    CREATE VIEW IF NOT EXISTS laws AS
                    SELECT rowid AS id, law_name
                    FROM assembly_laws
                    """
                )
            except Exception as e:
                logger.warning(f"Failed to create view 'laws': {e}")

            # Minimal tables for amendments and case_citations if absent
            cursor.execute(
                """
