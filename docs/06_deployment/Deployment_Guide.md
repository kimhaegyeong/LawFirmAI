...
`streamlit/Dockerfile` 파일이 이미 준비되어 있습니다:
...
# 컨테이너 실행
docker run -p 8501:8501 lawfirm-ai
```

### 2. Docker Compose 배포

`streamlit/docker-compose.yml` 파일을 사용:
...
```
lawfirmai/
├── streamlit/app.py       # Streamlit 앱 메인 파일
...
ExecStart=/home/ubuntu/lawfirmai/venv/bin/streamlit run streamlit/app.py --server.port=8501
Restart=always
