# 시작하기 (Getting Started)

이 섹션은 LawFirmAI 프로젝트를 시작하는 데 필요한 정보를 제공합니다.

## 📋 문서 목록

### 프로젝트 개요
- [프로젝트 개요](project_overview.md): LawFirmAI 프로젝트의 목표, 완성된 기능, 기술적 개선사항 및 향후 계획
- [프로젝트 구조](project_structure.md): 프로젝트의 전체 디렉토리 구조 및 각 모듈의 역할
- [아키텍처](architecture.md): LawFirmAI 서비스 아키텍처 및 핵심 컴포넌트

## 🚀 빠른 시작

1. [프로젝트 개요](project_overview.md)를 읽어 프로젝트의 목표와 기능을 이해하세요
2. [프로젝트 구조](project_structure.md)를 확인하여 코드베이스의 구조를 파악하세요
3. [아키텍처](architecture.md)를 통해 시스템의 전체적인 설계를 이해하세요

## 🔗 관련 섹션

- [02_Data](../02_data/collection/README.md): 데이터 수집 및 처리
- [03_RAG_System](../03_rag_system/README.md): RAG 시스템
- [06_Deployment](../06_deployment/README.md): 배포 가이드
- [09_User_Guide](../09_user_guide/README.md): 사용자 가이드
- [10_Technical_Reference](../10_technical_reference/README.md): 기술 참고 문서
