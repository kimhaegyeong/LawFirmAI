# -*- coding: utf-8 -*-
"""
LawFirmAI - 초간단 ChatGPT 스타일
확실히 보이는 사이드바와 깔끔한 디자인
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('logs/simple_chatgpt_app.log')
    ]
)
logger = logging.getLogger(__name__)

class SimpleChatGPTStyleLawFirmAI:
    """초간단 ChatGPT 스타일 LawFirmAI 애플리케이션"""
    ...
    def _initialize_components(self):
        """컴포넌트 초기화"""
        try:
            self.logger.info("Initializing simple ChatGPT-style components...")
            
            config = Config()
            self.chat_service = ChatService(config)
            
            self.is_initialized = True
            self.logger.info("Simple ChatGPT-style components initialized successfully")
...
def create_simple_chatgpt_interface():
    """초간단 ChatGPT 스타일 Gradio 인터페이스 생성"""
    
    # 앱 인스턴스 생성
    app = SimpleChatGPTStyleLawFirmAI()
    ...
    with gr.Blocks(
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                # 채팅 헤더
                chat_title = gr.HTML("<h1 style='color: #ececf1; font-size: 18px; font-weight: 600; margin: 0; padding: 16px 24px; border-bottom: 1px solid #2f2f2f; background: #212121;'>새 대화</h1>")
...
def main():
    """메인 함수"""
    logger.info("Starting LawFirmAI Simple ChatGPT-style application...")
    
    # Gradio 인터페이스 생성 및 실행
    interface = create_simple_chatgpt_interface()
