...
ê¸°ì¡´ 272ê°œì—??800ê°œë¡œ ?•ì¥?˜ì—¬ ???¤ì–‘??ë²•ë¥  ?„ë©”?¸ì„ ?¬í•¨?©ë‹ˆ??
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/expanded_training_data_generation.log'),
...
class ExpandedTrainingDataGenerator:
    """?•ì¥???ˆë ¨ ?°ì´???ì„±ê¸?""
    ...
    def __init__(self, target_size: int = 800):
        ...
        self.extended_law_domains = {
            "ë¯¼ë²•": {
                ...
                "ë¶ˆë²•?‰ìœ„": [
                    ...
                    ("ê³µì‘ë¬¼ì˜ ì±…ì„", "ê³µì‘ë¬¼ì˜ ?¤ì¹˜??ë³´ì¡´???˜ìë¡??¸í•œ ?í•´", "ë¯¼ë²• ??58ì¡?)
                ],
                "?ì†ë²?: [
            ...
            "?ë²•": {
                ...
                "?´ìŒ?˜í‘œë²?: [
                    ...
                    ("?´ìŒ??? ì¸", "ë§Œê¸° ?„ì— ?´ìŒ??? ì¸", "?´ìŒë²???8ì¡?),
                    ("?´ìŒ???´ë³´", "?´ìŒ???´ë³´ë¡??œê³µ", "?´ìŒë²???0ì¡?)
                ],
                "ë³´í—˜ë²?: [
            ...
            "?•ë²•": {
                ...
                "?? ¥ë²?: [
                    ("?í•´??êµ¬ì„±?”ê±´", "?¬ëŒ??? ì²´ë¥??í•´?˜ëŠ” ?‰ìœ„", "?•ë²• ??57ì¡?),
                    ("??–‰??êµ¬ì„±?”ê±´", "?¬ëŒ??? ì²´???€?˜ì—¬ ??–‰", "?•ë²• ??60ì¡?),
                    ("?‘ë°•??êµ¬ì„±?”ê±´", "?¬ëŒ???‘ë°•?˜ëŠ” ?‰ìœ„", "?•ë²• ??83ì¡?),
                    ("ê°•ìš”??êµ¬ì„±?”ê±´", "??–‰ ?ëŠ” ?‘ë°•?¼ë¡œ ?˜ë¬´ ?†ëŠ” ?¼ì„ ?˜ê²Œ ??, "?•ë²• ??24ì¡?),
                    ("ê°ê¸ˆ??êµ¬ì„±?”ê±´", "?¬ëŒ??ê°ê¸ˆ?˜ëŠ” ?‰ìœ„", "?•ë²• ??76ì¡?),
                    ("?½ì·¨? ì¸??êµ¬ì„±?”ê±´", "?¬ëŒ???½ì·¨?˜ê±°??? ì¸", "?•ë²• ??87ì¡?),
                    ("ê°•ê°„??êµ¬ì„±?”ê±´", "??–‰ ?ëŠ” ?‘ë°•?¼ë¡œ ê°„ìŒ", "?•ë²• ??97ì¡?),
            ...
            "?¸ë™ë²?: {
                "ê·¼ë¡œê¸°ì?ë²?: [
                    ...
                    ("?°ì—…?¬í•´", "?…ë¬´???¬í•´ë¡??¸í•œ ?í•´", "?°ì—…?¬í•´ë³´ìƒë³´í—˜ë²???ì¡?),
        ...
        self.extended_precedent_cases = [
            ...
            },
            {
                ...
                "case_summary": "êµí†µ?¬ê³  ê³¼ì‹¤?ê³„",
                "ruling": "êµí†µ?¬ê³ ?ì„œ ?¼í•´?ì˜ ê³¼ì‹¤???ˆëŠ” ê²½ìš° ê³¼ì‹¤?ê³„ë¥??ìš©?˜ì—¬ ?í•´ë°°ìƒ?¡ì„ ê°ì•¡?????ˆìŠµ?ˆë‹¤.",
                "law_applied": "ë¯¼ë²• ??63ì¡?,
                "court": "?€ë²•ì›",
                "date": "2024.05.15"
            },
        ...
        logger.info(f"ExpandedTrainingDataGenerator initialized with target size: {target_size}")
    
    def generate_extended_law_qa_pairs(self, target_count: int) -> List[Dict[str, Any]]:
        """?•ì¥??ë²•ë ¹ ?°ì´?°ë¡œë¶€??Q&A ?ì„±"""
        qa_pairs = []
        id_counter = 1
        
        for law_name, topics in self.extended_law_domains.items():
            for topic_name, qa_data in topics.items():
                for question_part, answer_part, article in qa_data:
                    # ê¸°ë³¸ ?•ì˜ Q&A
                    qa_pairs.append({
                        "id": f"ext_law_{id_counter:03d}",
                        ...
                        "metadata": {
                            ...
                            "topic": topic_name,
                            "article": article,
                            "generated_from": "extended_law_template"
                        }
                    })
                    id_counter += 1
                    ...
                    qa_pairs.append({
                        ...
                        "metadata": {
                            ...
                            "topic": topic_name,
                            "article": article,
                            "generated_from": "extended_article_interpretation"
                        }
                    })
                    id_counter += 1
                    ...
                    qa_pairs.append({
                        ...
                        "metadata": {
                            ...
                            "topic": topic_name,
                            "article": article,
                            "generated_from": "extended_application_example"
                        }
                    })
                    id_counter += 1
                    ...
                    qa_pairs.append({
                        ...
                        "metadata": {
                            ...
                            "topic": topic_name,
                            "article": article,
                            "generated_from": "extended_practice_guidance"
                        }
                    })
                    id_counter += 1
    ...
    def generate_extended_precedent_qa_pairs(self, target_count: int) -> List[Dict[str, Any]]:
        """?•ì¥???ë? ?°ì´?°ë¡œë¶€??Q&A ?ì„±"""
        qa_pairs = []
        id_counter = 1
        
        for case in self.extended_precedent_cases:
            # ?¬ê±´ ?”ì•½ Q&A
            qa_pairs.append({
                "id": f"ext_precedent_{id_counter:03d}",
                ...
                "metadata": {
                    ...
                    "generated_from": "extended_case_summary"
                }
            })
            id_counter += 1
            ...
            qa_pairs.append({
                ...
                "metadata": {
                    ...
                    "date": case["date"],
                    "law_applied": case["law_applied"],
                    "generated_from": "extended_legal_principle"
                }
            })
            id_counter += 1
            ...
            qa_pairs.append({
                ...
                "metadata": {
                    ...
                    "date": case["date"],
                    "law_applied": case["law_applied"],
                    "generated_from": "extended_similar_cases"
                }
            })
            id_counter += 1
            ...
            qa_pairs.append({
                ...
                "metadata": {
                    ...
                    "date": case["date"],
                    "law_applied": case["law_applied"],
                    "generated_from": "extended_legal_impact"
                }
            })
            id_counter += 1
            ...
            qa_pairs.append({
                ...
                "answer": f"{case['case_number']} ?ê²°?€ {case['case_summary']}?€ ê´€?¨ëœ ?¬ê±´?ì„œ '{case['ruling']}'??ë²•ë¦¬ë¥??ìš©?????ˆëŠ” ì¤‘ìš”??ê¸°ì????©ë‹ˆ?? ?¤ë¬´?ì„œ?????ê²°??ë²•ë¦¬ë¥?ì°¸ê³ ?˜ì—¬ ? ì‚¬???¬ê±´??ì²˜ë¦¬?????ˆìŠµ?ˆë‹¤.",
                ...
                "metadata": {
                    ...
                    "date": case["date"],
                    "law_applied": case["law_applied"],
                    "generated_from": "extended_practice_application"
                }
            })
            id_counter += 1
                ...
                **qa,
                ...
                "metadata": {
                    **qa.get("metadata", {}),
                    ...
                    "format": "kogpt2_training_extended"
                }
            })
    ...
    def generate_expanded_dataset(self) -> Dict[str, Any]:
        """?•ì¥???ˆë ¨ ?°ì´?°ì…‹ ?ì„±"""
        logger.info("?•ì¥???ˆë ¨ ?°ì´?°ì…‹ ?ì„± ?œì‘...")
        
        # ë²•ë ¹ Q&A ?ì„± (60%)
        law_count = int(self.target_size * 0.6)
        logger.info(f"ë²•ë ¹ Q&A ?ì„±: {law_count}ê°?)
        law_qa_pairs = self.generate_extended_law_qa_pairs(law_count)
        
        # ?ë? Q&A ?ì„± (40%)
        precedent_count = int(self.target_size * 0.4)
        logger.info(f"?ë? Q&A ?ì„±: {precedent_count}ê°?)
        precedent_qa_pairs = self.generate_extended_precedent_qa_pairs(precedent_count)
...
def main():
    """ë©”ì¸ ?¤í–‰ ?¨ìˆ˜"""
    logger.info("?•ì¥???ˆë ¨ ?°ì´???ì„± ?œì‘...")
    
    # ?•ì¥???ˆë ¨ ?°ì´???ì„±ê¸?ì´ˆê¸°??
    generator = ExpandedTrainingDataGenerator(target_size=800)
    
    # ?•ì¥???°ì´?°ì…‹ ?ì„±
    result = generator.generate_expanded_dataset()
    
    # ê²°ê³¼ ì¶œë ¥
    print("\n" + "="*60)
    print("?“Š ?•ì¥???ˆë ¨ ?°ì´?°ì…‹ ?ì„± ê²°ê³¼")
    print("="*60)
