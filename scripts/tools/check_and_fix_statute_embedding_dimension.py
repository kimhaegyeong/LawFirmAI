"""
statute_embeddings 테이블의 실제 임베딩 차원 확인 및 embedding_versions 테이블 정보 수정

이 스크립트는:
1. statute_embeddings 테이블의 실제 임베딩 벡터 차원을 확인
...
def check_embedding_dimensions(engine) -> Dict[int, Dict[str, Any]]:
    """statute_embeddings 테이블의 실제 임베딩 차원 확인"""
    print("="*80)
    print("1. statute_embeddings 테이블 임베딩 차원 확인")
    print("="*80)
    ...
    with engine.connect() as conn:
        ...
        for version in versions:
            ...
            if sample_row:
                ...
                print(f"  version {version}: 차원 {dimension} ({count:,}개 임베딩)")
        
        print("\n  전체 차원 분포:")
        for dim, count in sorted(dimensions_count.items()):
            print(f"    차원 {dim}: {count:,}개 임베딩")
...
def main():
    ...
    if not versions:
        print("\n❌ statutes 타입의 embedding_versions가 없습니다.")
        return
    ...
    for version_info in versions:
        ...
        if version_num in version_dimensions:
            ...
            print(f"    실제 차원: {actual_dim} (임베딩 수: {actual_count:,})")
