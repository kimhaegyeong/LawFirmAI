...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/real_raw_data_rebuild.log', encoding='utf-8'),
    ...
    try:
        ...
        for file_path in json_files:
            try:
                ...
                # 파일 파싱
                laws = parser.parse_raw_file(file_path)
                ...
                logger.info(f"Successfully processed {file_path.name}: {len(laws)} laws")
...
def main():
    """메인 함수"""
    logger.info("Starting Real Raw Data Database Rebuild")
    logger.info("="*60)
    ...
    try:
        ...
        logger.info(f"  - Successfully Inserted: {results['successful_inserts']}")
        ...
        report_path = Path("data/real_raw_data_rebuild_report.json")
        with open(report_path, 'w', encoding='utf-8') as f:
            json.dump(report, f, ensure_ascii=False, indent=2)
        logger.info(f"Report saved to: {report_path}")
        
        logger.info("="*60)
        logger.info("Real Raw Data Database Rebuild Completed!")
        
        return 0 if results['successful_inserts'] > 0 else 1
