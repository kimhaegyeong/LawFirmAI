...
국가법령정보센터 LAW OPEN API를 사용하여 판례를 수집합니다.
- 최근 5년간 판례 5,000건 수집
- 판례명 검색, 본문 검색, 날짜 범위 검색 활용
- 사건번호, 법원, 판결일 등 메타데이터 정제
- 판례 카테고리 분류 (민사, 형사, 행정, 상사 등)
...
def main():
    ...
    try:
        ...
        if not oc:
            ...
            logger.info("사용법: LAW_OPEN_API_OC=your_email_id python collect_precedents.py")
            return 1
        ...
        if len(sys.argv) > 1:
            if sys.argv[1] == "--check" or sys.argv[1] == "-c":
                ...
                # 도움말 출력
                print("판례 수집 스크립트 사용법 (우선순위 기반):")
                print("  python collect_precedents.py [옵션] [목표수량]")
                print("")
                print("옵션:")
                ...
                print("")
                print("수집 방식:")
                print("  - 우선순위 키워드 우선 수집 (계약, 손해배상, 사기 등)")
                print("  - 키워드별 차등 목표 건수 (우선순위: 15-100건, 일반: 15건)")
                print("  - 총 200개 이상 키워드로 체계적 수집")
                print("  - 이미 처리된 키워드는 자동으로 건너뛰기")
                print("")
                print("예시:")
                print("  python collect_precedents.py              # 기본 5,000건 수집")
                print("  python collect_precedents.py 10000        # 10,000건 수집")
                print("  python collect_precedents.py --check      # 진행 상황 확인")
                return 0
            else:
                try:
                    ...
                    logger.warning(f"잘못된 목표 수량: {sys.argv[1]}, 기본값 사용: 5000건")
                    target_count = 5000
        else:
            target_count = 5000
        ...
        logger.info("판례 수집이 성공적으로 완료되었습니다.")
        return 0
