...
# scripts/ingest/open_law/scripts/collect_administrative_statutes.py -> 프로젝트 루트
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/open_law/administrative_statutes_collection.log', encoding='utf-8'),
...
def main():
    parser = argparse.ArgumentParser(description='행정법 현행법령 수집')
    parser.add_argument(
        '--oc',
    ...
    parser.add_argument(
        '--output',
        default='data/raw/open_law/administrative_statutes_list.json',
    ...
    if args.phase == 'list':
        ...
        logger.info(f"행정법 법령 목록 수집 완료: {len(unique_statutes)}개")
        ...
        logger.info(f"행정법 법령 본문 수집 완료: {len(statutes)}개 법령, {total_articles}개 조문")
