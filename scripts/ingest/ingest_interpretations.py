...
def insert_paragraphs(conn: sqlite3.Connection, interp_id: int, paragraphs: List[str]) -> List[int]:
    ids: List[int] = []
    for i, p in enumerate(paragraphs):
        cur = conn.execute(
            "INSERT OR REPLACE INTO interpretation_paragraphs(interpretation_id, para_index, text) VALUES(?,?,?)",
            (interp_id, i, p),
        )
        ids.append(cur.lastrowid if cur.lastrowid else conn.execute(
            "SELECT id FROM interpretation_paragraphs WHERE interpretation_id=? AND para_index=?", (interp_id, i)
        ).fetchone()[0])
    return ids
...
def main():
    ...
    with sqlite3.connect(args.db) as conn:
        ...
        interp_id = insert_interpretation(conn, domain_id, meta)
        insert_paragraphs(conn, interp_id, meta.get("sentences", []))
        conn.commit()

        embedder = SentenceEmbedder(args.model)
        insert_chunks_and_embeddings(conn, interp_id, meta.get("sentences", []), embedder)
        conn.commit()

    print("Ingested interpretation:", meta.get("doc_id"))
