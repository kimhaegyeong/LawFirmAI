# -*- coding: utf-8 -*-
"""
dlytrm (일상용어) API 데이터 수집 스크립트
국가법령정보 공동활용 LAW OPEN DATA - 일상용어 검색 데이터 수집
"""
...
def _create_table(conn):
    ...
    cursor.execute("""
        CREATE INDEX IF NOT EXISTS idx_dlytrm_term_id 
        ON open_law_dlytrm_data(term_id)
    """)
    
    cursor.execute("""
        CREATE INDEX IF NOT EXISTS idx_dlytrm_keyword 
        ON open_law_dlytrm_data(search_keyword)
    """)
    
    cursor.execute("""
        CREATE INDEX IF NOT EXISTS idx_dlytrm_collected_at 
        ON open_law_dlytrm_data(collected_at)
    """)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='dlytrm (일상용어) API 데이터 수집')
    parser.add_argument(
        '--oc',
    ...
    # API 클라이언트 및 수집기 생성
    client = DlytrmClient(args.oc)
    client.rate_limit_delay = args.rate_limit
    
    collector = DlytrmCollector(client, args.db_path)
