...
class FastLawPreprocessor:
    ...
    def preprocess_directory(self, input_dir: Path, output_dir: Path, max_workers: int = None) -> Dict[str, Any]:
        ...
        logger.info(f"?ìÅ Files: {len(json_files)}")
        logger.info(f"??Workers: {max_workers}")
        logger.info(f"?ì¶ Batch size: {self.batch_size}")
        ...
        logger.info("=" * 60)
        logger.info(f"?ìÅ Total files: {summary['total_files']}")
        logger.info(f"??Successful: {summary['successful_files']}")
        logger.info(f"?ìä Success rate: {summary['success_rate']:.1f}%")
        logger.info(f"?ìú Total laws: {summary['total_laws']}")
        logger.info(f"??Processed laws: {summary['processed_laws']}")
        logger.info(f"?±Ô∏è  Total time: {summary['total_time']:.2f} seconds")
        logger.info(f"?? Processing rate: {summary['processing_rate']:.2f} laws/second")
        logger.info(f"?ë• Workers used: {summary['workers_used']}")
        logger.info(f"?ì¶ Batch size: {summary['batch_size']}")
...
def main():
    ...
    logger.info(f"?ìÅ Input: {input_dir}")
    logger.info(f"?ìÅ Output: {output_dir}")
    ...
    try:
        ...
        if summary['success']:
            logger.info("?éâ Fast preprocessing completed successfully!")
            return 0
        else:
        ...
        logger.info("?†Ô∏è Processing interrupted by user")
        return 1
    except Exception as e:
        logger.error(f"?í• Unexpected error: {str(e)}")
        return 1
