...
"""
ë¶€ì¹??Œì‹± ê°œì„  ?¤í¬ë¦½íŠ¸
?€?œë?êµ?ë²•ë¥  ë¶€ì¹??‘ì„± ê·œì¹™???°ë¥¸ ë¶€ì¹??¸ì‹ ë°??Œì‹± ê°œì„ 
"""
...
class EnhancedArticleParser(ImprovedArticleParser):
    ...
    def _identify_supplementary_section(self, content: str) -> tuple:
        ...
        # ë¶€ì¹??ì  ì°¾ê¸° (?¤ìŒ ?¹ì…˜?´ë‚˜ ë¬¸ì„œ ??
        for i in range(supplementary_start + 1, len(lines)):
            line_clean = lines[i].strip()
            
            # ë¶€ì¹™ì´ ?„ë‹Œ ?¤ë¥¸ ?¹ì…˜ ?œì‘ ?•ì¸
            if line_clean and not self._is_supplementary_line(line_clean):
                # ë¶€ì¹™ì´ ?„ë‹Œ ?ˆë¡œ???¹ì…˜?¸ì? ?•ì¸
                if re.match(r'??d+ì¡?, line_clean) and not self._is_supplementary_article(line_clean):
                    supplementary_end = i
                    break
...
??6ì¡°ì œ1??ì¤?"8?œê°„"??"7?œê°„"?¼ë¡œ ?œë‹¤."""
    ...
    print(f"ë³¸ì¹™ ì¡°ë¬¸ ?? {result['total_articles'] - result.get('total_supplementary_articles', 0)}")
    print(f"ë¶€ì¹?ì¡°ë¬¸ ?? {result.get('total_supplementary_articles', 0)}")
    print()
