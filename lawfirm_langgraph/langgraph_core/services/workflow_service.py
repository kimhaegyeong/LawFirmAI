...
from langgraph_core.services.legal_workflow_enhanced import (
...
try:
    ...
    from langgraph_core.services.langfuse_client import LangfuseClient
...
from langgraph_core.utils.state_definitions import create_initial_legal_state
...
class LangGraphWorkflowService:
    ...
    def __init__(self, config: Optional[LangGraphConfig] = None):
        ...
        if enable_checkpoint:
            try:
                ...
                self.logger.info("Checkpoint enabled with SQLite storage")
            except Exception as e:
                self.logger.warning(f"Checkpoint initialization failed: {e}. Using None.")
                checkpointer = None
                self.checkpoint_manager = None
        else:
            checkpointer = None
            self.checkpoint_manager = None
            self.logger.info("Checkpoint disabled")
        ...
        if not enable_langsmith:
            ...
            try:
                ...
                checkpoint_status = "with checkpoint" if checkpointer else "without checkpoint"
                self.logger.info(f"워크플로우가 {checkpoint_status}로 컴파일되었습니다 (LangSmith 비활성화됨)")
            finally:
                # 환경 변수 복원
                if original_tracing:
            ...
            checkpoint_status = "with checkpoint" if checkpointer else "without checkpoint"
            self.logger.info(f"워크플로우가 LangSmith 추적으로 컴파일되었습니다 ({checkpoint_status}, State Reduction 적용됨)")
    ...
    async def process_query(
        ...
        try:
            ...
            config = {}
            if self.checkpoint_manager and enable_checkpoint:
            ...
            if self.app:
                ...
                if isinstance(flat_result, dict):
                    ...
                    try:
                        import sys
                        node_wrappers_module = sys.modules.get('source.models.node_wrappers')
    ...
    def _extract_retrieved_docs_from_result(self, flat_result: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            from langgraph_core.models.node_wrappers import _global_search_results_cache
