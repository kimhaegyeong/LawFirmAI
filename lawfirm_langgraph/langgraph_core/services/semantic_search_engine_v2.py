...
class SemanticSearchEngineV2:
    ...
    def _load_chunk_vectors(self,
        ...
        try:
            ...
            if self._is_table_error(e):
                self.logger.warning(f"Table 'embeddings' or related tables not found: {e}. "
                                  f"Database may need migration or embeddings generation. "
    ...
    def _load_chunk_vectors_batch(self,
        ...
        try:
            ...
            # 테이블이 없는 경우는 warning으로 처리 (정상적인 초기 상태일 수 있음)
            if self._is_table_error(e):
                self.logger.warning(f"Table 'embeddings' not found in batch load: {e}. "
                                  f"Database may need migration or embeddings generation. "
                                  f"Returning empty results.")
            else:
