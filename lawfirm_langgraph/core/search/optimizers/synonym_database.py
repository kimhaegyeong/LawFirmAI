...
class SynonymDatabase:
    ...
    def _initialize_database(self):
        ...
        self.logger.info("동의어 데이터베이스 초기화 완료: PostgreSQL")
        ...
        try:
            with self._get_connection_context() as conn:
                ...
                query = '''
                    ...
                    WHERE keyword = %s AND synonym = %s
                '''
                params = [datetime.now(), keyword, synonym]
    ...
    def update_user_rating(self, keyword: str, synonym: str, rating: float,
        ...
        try:
            with self._get_connection_context() as conn:
                ...
                row = cursor.fetchone()
                if row:
                    old_rating = float(row['user_rating'])
                    usage_count = int(row['usage_count'])
                    if old_rating > 0:
                    ...
                    query = '''
                        UPDATE synonyms 
                        SET user_rating = %s, usage_count = usage_count + 1
                        WHERE keyword = %s AND synonym = %s
                    '''
    ...
    def get_database_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "average_usage_count": round(float(avg_usage), 2),
...
if __name__ == "__main__":
    ...
    # 데이터베이스 초기화
    db = SynonymDatabase(database_url=database_url)
