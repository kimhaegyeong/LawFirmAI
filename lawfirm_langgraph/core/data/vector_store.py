...
# PyTorch 보안 취약점 회피를 위한 환경 변수 설정
import os
import pickle
import threading
import time
...
class LegalVectorStore:
    ...
    def search(self, query: str, top_k: int = 10, filters: Dict = None, enhanced: bool = True) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # 결과 처리
            results = []
            confidence_threshold = 0.6  # 신뢰도 60% 미만 필터링
            filtered_count = 0
            ...
            for score, idx in zip(scores[0], indices[0]):
                ...
                if ml_confidence is not None and ml_confidence < confidence_threshold:
                    filtered_count += 1
                    continue
            ...
            if filtered_count > 0:
                self.logger.info(f"Filtered out {filtered_count} documents with confidence < {confidence_threshold}")
