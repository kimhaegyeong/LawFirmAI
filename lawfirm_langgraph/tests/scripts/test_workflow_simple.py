...
async def test_workflow_simple():
    ...
    try:
        ...
        print(f"\n📝 테스트 쿼리: {test_query}")
        print("\n⏳ 워크플로우 실행 중... (체크포인트 비활성화)")
        
        # 워크플로우 실행 (체크포인트 비활성화하여 numpy 직렬화 오류 방지)
        ...
        if isinstance(result, dict):
            ...
            # 결과를 파일로 저장
            import json
            output_file = "workflow_test_result_simple.json"
...
def main():
    """메인 함수"""
    try:
        result = asyncio.run(test_workflow_simple())
        return result
    except Exception as e:
        print(f"\n❌ 테스트 실행 실패: {e}")
        import traceback
